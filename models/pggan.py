...
class Generator(nn.Module):
    def __init__(self, input_code_dim=128, in_channel=128, pixel_norm=True, tanh=True):
        ...
        progress_layers = [self.progression_4, self.progression_8,self.progression_16,self.progression_32,self.progression_64,self.progression_128, self.progression_256]
        rgb_layers = [self.to_rgb_8, self.to_rgb_16, self.to_rgb_32, self.to_rgb_64, self.to_rgb_128, self.to_rgb_256]
        
        self.rgb = nn.ModuleList(rgb_layers)
        self.progression = nn.ModuleList(progress_layers)
        ...
        if index == 0:
            ...
            if update_embed:
                for param in self.rgb.parameters():
                    yield param
        elif index < len(self.progression) + 1:
            for param in self.progression[index - 1].parameters():
                yield param
        elif index == len(self.progression) + 1:
